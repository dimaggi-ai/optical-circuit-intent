...
CASES: Tuple[Tuple[Tuple[str, ...], int, str], ...] = (
    ...
    (("tapi", "examples/tapi-request.json", "--sip-table", "examples/tapi-sip-table.json",
      "--slot-width-ghz", "50", "--now", "2026-09-01T00:00:00Z"), 0,
     "a request as the calls a TR-547 controller documents: two reads, one create, two reads back"),
    (("tapi", "examples/tapi-request.json", "--sip-table", "examples/tapi-sip-table.json",
      "--slot-width-ghz", "50", "--now", "2026-09-01T00:00:00Z", "--json"), 0,
     "the same plan as JSON, bodies and expectations included"),
    (("tapi", "examples/tapi-failover.json", "--sip-table", "examples/tapi-sip-table.json",
      "--slot-width-ghz", "50", "--now", "2026-09-01T00:00:00Z"), 0,
     "a failover: the one DELETE comes last, after the read that verifies the replacement"),
    (("tapi", "examples/tapi-hold-until.json", "--sip-table", "examples/tapi-sip-table.json",
      "--now", "2026-09-01T00:00:00Z"), 0,
     "a hold extension without the service object: the read, and the PUT named as unmapped"),
    (("tapi", "examples/tapi-hold-until.json", "--sip-table", "examples/tapi-sip-table.json",
      "--now", "2026-09-01T00:00:00Z", "--current-service", "examples/tapi-current-service.json"), 0,
     "the same hold with the object: a PUT of the whole service with only its end time changed"),
    (("tapi", "examples/tapi-release.json", "--sip-table", "examples/tapi-sip-table.json",
      "--now", "2026-09-01T00:00:00Z"), 0,
     "a release: one DELETE, the only destructive call the binding ever emits"),
    (("tapi", "examples/tapi-request-unknown-port.json", "--sip-table", "examples/tapi-sip-table.json",
      "--now", "2026-09-01T00:00:00Z"), 1,
     "an endpoint the SIP table does not know: refused, with no call at all"),
    (("tapi", "examples/tapi-request.json", "--sip-table", "examples/no-such-table.json"), 2,
     "a SIP table that cannot be read: unreadable, not admitted"),
)
